...
class Book {
  ...
  changeReadStatus() {
    if (this.isReadIt === "Yes") this.isReadIt = "No";
    else this.isReadIt = "Yes";
  }
}
  ...
  for (let i = 0; i < totalBooks; i++) {
    if (myLibrary[i].id === id) {
      ...
      console.log(
        "The book " +
          myLibrary[i].title +
          " was erased. The library now have " +
          totalBooks +
          " books in total"
      );
...
function displayBooks() {
  ...
  for (let i = 0; i < totalBooks; i++) {
    ...
    remove_book_Button.addEventListener("click", () => {
      console.log(
        "Removing the book with id " + remove_book_Button.getAttribute("data")
      );
      removeBook(remove_book_Button.getAttribute("data"));
    });
    ...
    change_status_Button.addEventListener("click", () => {
      console.log(
        "Changing the status of the book with id " +
          change_status_Button.getAttribute("data")
      );
      changeStatusButton(change_status_Button.getAttribute("data"));
    });
...
const input_AuthorName = document.getElementById("author_name");
input_AuthorName.required = true;

input_AuthorName.addEventListener("input", (event) => {
  if (input_AuthorName.validity.valueMissing) {
    addBook = false;
    console.log(input_AuthorName);
    input_AuthorName.setCustomValidity("The author name must be filled!");
  } else {
    addBook = true;
    input_AuthorName.setCustomValidity("");
  }
});
